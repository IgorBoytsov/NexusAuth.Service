...
namespace NexusAuth.Application.Helpers
{
    public static class RequestGuard
    {
        public static bool TryGetFailureResult<TRequest>(TRequest? request, out Result failureResult)
        {
            ...
            failureResult = Result.Failure(new Error(ErrorCode.NullValueCommand, systemMessage, clientMessage));
            return true;
        }
        ...
        public static bool TryGetFailureResult<TRequest, TResult>(TRequest? request, out Result<TResult> failureResult)
        {
            ...
            failureResult = Result<TResult>.Failure(new Error(ErrorCode.NullValueCommand, systemMessage, clientMessage));
            return true;
        }
    }
}